...
func main() {
	...
	outPath := reportOutputPath(st)
	if err := analyzeAndReport(d, st, outPath); err != nil {
		fatalf("analyze failed: %v", err)
	}
	fmt.Fprintf(os.Stderr, "Отчёт сохранён: %s\n", outPath)
}

func reportOutputPath(st settings) string {
	base := strings.TrimSuffix(filepath.Base(st.FilePath), filepath.Ext(st.FilePath))
	if base == "" {
		base = "analysis"
	}
	return filepath.Join("./results", fmt.Sprintf("%s-%s.html", base, st.Mode))
}

func analyzeAndReport(d *docline.Docline, st settings, outPath string) error {
	switch st.Mode {
	case modeAutomatic:
		convert := st.ConvertToDRL
		archetype := st.ArchetypeLength
		strict := st.StrictFilter
		result, err := d.AnalyzeDocument(st.FilePath, modeAutomatic, docline.AutomaticConfig{
		...
		return d.GenerateReport(result, "html", outPath)
	case modeHeuristic:
		result, err := d.AnalyzeDocument(st.FilePath, modeHeuristic, docline.HeuristicConfig{
			MinCloneLength:      st.MinCloneLength,
			MinGroupPower:       st.MinGroupPower,
		...
		return d.GenerateReport(result, "html", outPath)
	case modeNgram:
		result, err := d.AnalyzeDocument(st.FilePath, modeNgram, docline.NgramConfig{
			MinCloneLength: st.MinCloneLength,
			MinGroupPower:  st.MinGroupPower,
			MaxEdit:        st.MaxEdit,
		...
		return d.GenerateReport(result, "html", outPath)
	case modeOpenAI:
		result, err := d.AnalyzeDocument(st.FilePath, modeOpenAI, docline.OpenAIConfig{
			MinGroupPower:     st.MinGroupPower,
